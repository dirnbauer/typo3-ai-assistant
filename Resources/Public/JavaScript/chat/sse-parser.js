...
/** The events a turn can emit. Anything else is dropped, not fatal. */
export const TURN_EVENTS = Object.freeze([
  'run.started',
  'step.llm',
  'step.tool.call',
  'step.tool.result',
...
const KNOWN = new Set(TURN_EVENTS);

/**
 * @typedef {{event: string, data: string, id?: number}} Frame
 * @typedef {{event: string, data: unknown, id?: number}} TurnEvent
 */
...
export class SseParser {
  ...
  /**
   * @param {string} chunk
   * @param {boolean} final true when the body has ended and nothing more can arrive
   * @returns {Frame[]} every complete frame the added text finished, in order
   */
  push(chunk, final = false) {
    ...
    let rest = body.replace(/\r/g, '\n');

    const frames = [];
    for (let boundary = rest.indexOf('\n\n'); boundary !== -1; boundary = rest.indexOf('\n\n')) {
 ...
 *
 * @param {Frame} frame
 * @returns {TurnEvent|null}
 */
export function toTurnEvent(frame) {
  if (!KNOWN.has(frame.event)) {
    return null;
  }
 ...
 * @param {(event: TurnEvent) => void} onEvent
